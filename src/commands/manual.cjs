...
const SECTIONS = {
  ...
  container: {
    ...
    usage: 'lo container <promote|status|scan|list> [选项...]',
    description: [
      '容器管理命令集，提供容器成员的提升/降级、状态查看、同步和列表功能。',
      '',
      '子命令:',
      '',
      ...
      '  对比 Content Source 目录中的实际文件与 container_members 表。',
      '  展示新增 (A)、修改 (M)、删除 (D) 的文件列表。',
      '  如果成员已被提升，会标注 [已提升]。',
      '',
      'lo container scan <rid>',
      '  同步容器成员：将 Content Source 目录的变化应用到数据库。',
      '  新增数据库中没有的文件、更新 hash 变化的文件、移除已不存在的成员。',
      '  本质上是 lo container status + 应用变更。',
      '',
      'lo container list <rid>',
      '  列出容器的所有成员。',
      '  --resources 仅显示已提升的，--files 仅显示未提升的。',
      '',
      '工作机制:',
      '',
      ...
      '  Resource 后，才进入 lo 的资源管理管线（可打标签、建立 Relation）。',
      '',
      '选项 (promote):',
      '  --container, -c     容器 RID（不指定则自动查找）',
      '  --type, -t          目标 Resource 类型（默认根据扩展名推导，仅提升时生效）',
      '  --revert, -r        降级：将已提升成员恢复为普通文件成员',
      '',
      '选项 (list):',
      '  --resources         仅显示已提升为 Resource 的成员',
      '  --files             仅显示未提升的普通文件成员',
      '',
      '示例:',
      ...
      '  lo container list res_abc123                          # 列出所有成员',
      '  lo container list res_abc123 --resources              # 仅显示已提升的',
      '',
      '提升后的操作:',
