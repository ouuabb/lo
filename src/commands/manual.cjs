...
const SECTIONS = {
  ...
  container: {
    ...
    usage: 'lo container promote <路径> [--revert] [--container <rid>] [--type <类型>]',
    description: [
      '容器管理命令，当前支持提升和降级容器成员。',
      '',
      'lo container promote <path>（默认）',
      ...
      '',
      '  自动查找到容器:',
      '    如果不指定 --container，会自动遍历所有具有 container',
      '    capability 的 Resource，找到包含该文件路径的容器。',
      '',
      'lo container promote <path> --revert',
      '  将已提升的 Resource Member 降级为普通 File Member。',
      '',
      '  降级后:',
      '    成员不再关联独立 Resource（resource_rid 清空为 NULL）',
      '    Resource 本身不受影响（仍独立存在，不会被删除）',
      ...
      '  lo container promote src/main.py --type code          # 指定类型',
      '  lo container promote docs/design.md -c res_abc123     # 指定容器',
