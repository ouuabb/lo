...
const SECTIONS = {
  ...
  基本语法:
    [[目标标题]]             链接到标题为"目标标题"的笔记
    [[目标标题|显示别名]]     链接到"目标标题"，但显示为"显示别名"

  简单示例:

    笔记A.md 内容:                笔记B.md 内容:
    # 笔记A                        # 笔记B
    参考 [[笔记B]] 的思路。        来自 [[笔记A|小A]] 的启发。
  ...
  lo 自动完成了以下工作:
    1. 解析 [[...]] 语法，提取链接目标名称
    2. 按目标名称匹配其他 .md 文件的标题
    3. 匹配成功后写入数据库的 relations 表
  ...
  ┌──────────────────────────────────────────────────────────────┐
  │  3. 标题匹配                                                  │
  │     查询数据库: SELECT rid FROM resources                     │
  │                 WHERE type='note'                             │
  │     解析各笔记标题，匹配"笔记B" → rid_B, "笔记C" → rid_C       │
  │     未匹配的目标被静默忽略（不报错）                            │
  └──────────────────────┬───────────────────────────────────────┘
                         │
                         ▼
  ...
  │                   │                                          │
  │  按标题匹配       │  链接目标是笔记的标题（# 后的内容），     │
  │                   │  不是文件名或路径                          │
  │                   │                                          │
  │  不区分大小写     │  标题匹配是精确的（区分大小写）            │
  │                   │                                          │
