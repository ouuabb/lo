...
module.exports = function() {
  ...
  RID 在资源首次入库时生成（lo add + lo commit 或 lo sync），
  一旦分配则不可变更，伴随资源整个生命周期。`);
  ...
  │                  │ RID 恒定不变                         │
  ├──────────────────┼─────────────────────────────────────┤
  │ 名称解耦         │ name 是逻辑标签，可被栈层复用，      │
  │                  │ RID 才是终极唯一标识                 │
  ├──────────────────┼─────────────────────────────────────┤
  │ 关系稳定         │ relations 表通过 from_rid/to_rid     │
  │                  │ 建立链接，重命名/移动不影响链接      │
  ├──────────────────┼─────────────────────────────────────┤
  ...
  resolveResource(input) 实现了统一的资源查找入口：
    ...
    第 1 级: input 以 "res_" 开头 → getByRid(input)
             精确匹配，不区分 layer，栈中资源也能找到

    第 2 级: getByName(input)
             按 name 查找活跃层（layer=0）
             日常操作入口，只返回活跃资源

    第 3 级: getByPath(input)
             按文件路径匹配（含绝对路径和相对路径降级）

  这意味着用户可以自由使用 rid、name 或 path 中的任意一种方式
  引用资源，系统自动按优先级解析。`);
  ...
  日常使用中，用户更习惯用 name 引用资源（如 lo show 笔记测试），
  系统自动通过 resolveResource 查找活跃层（layer=0）返回。
  栈中资源则需要使用 RID 精确访问。`);
  ...
  所有 CRUD 命令（show/edit/delete/tag/category/link/unlink/move）
  统一使用 resolveResource 作为资源查找入口，确保一致性。`);
  ...
  相关命令: lo show, lo list, lo manual show
  相关文档: lo docs stack`);
  console.log('');
};
